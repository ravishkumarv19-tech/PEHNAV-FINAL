...
function AuthWall() {
  ...
  useEffect(() => {
    // Check URL parameters and hash for OAuth callback errors
    ...
    if (errorDesc || errorMsg) {
      const decoded = decodeURIComponent(errorDesc || errorMsg || "Failed to authenticate with Google");
      setError(decoded);
      toast.error(decoded);
    ...
    <div className="mx-auto max-w-sm px-6 py-20">
        ...
        <>
          ...
          <div className="space-y-2">
            <button
              ...
              disabled={loading}
              className="flex w-full items-center justify-center gap-2.5 rounded-md border border-border bg-card/60 py-3 text-sm font-medium transition-colors hover:bg-accent/50 disabled:opacity-60"
            >
            ...
            <button onClick={() => setMode("otp")} className="w-full rounded-md border border-border py-3 text-sm hover:bg-accent/30 transition-colors">Login with OTP / Email Link</button>
          </div>
      ...
      {(mode === "otp" || mode === "otp-verify") && (
        <form onSubmit={handle} className="mt-6 space-y-3">
          ...
          {mode === "otp-verify" && (
            <>
              <div className="rounded-md border border-gold/30 bg-gold/5 p-3 text-xs text-muted-foreground leading-relaxed">
                ✨ <strong>Check your email:</strong> You can either click the <strong>"Sign in"</strong> link in your email to log in instantly with 1-click, or enter the verification code below.
              </div>
              <input value={otp} onChange={(e) => setOtp(e.target.value.trim())} placeholder="Enter code from email" maxLength={10} required
                className="w-full rounded-md border border-border bg-card px-4 py-3 text-sm outline-none focus:border-gold text-center font-mono tracking-widest text-lg" />
            </>
          )}
          <button disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-md bg-foreground py-3 text-sm uppercase tracking-wider text-background disabled:opacity-60">
            {loading && <Loader2 className="h-4 w-4 animate-spin" />}
            {mode === "otp" ? "Send Login Link / OTP" : "Verify & Sign In"}
          </button>
    ...
    <div>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          {profile?.avatar_url ? (
            <img src={profile.avatar_url} alt={profile.full_name ?? "Avatar"} className="h-10 w-10 rounded-full border border-border object-cover" />
          ) : (
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gold/20 text-gold font-medium text-sm">
...
function Account() {
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-4 sm:px-6 py-6 sm:py-12 pb-28 sm:pb-12">
      <h1 className="font-display text-3xl sm:text-4xl font-bold">My Account</h1>
      <div className="mt-6 sm:mt-8 grid gap-6 lg:gap-8 lg:grid-cols-[220px_1fr]">
        <aside className="flex lg:flex-col overflow-x-auto no-scrollbar gap-2 pb-2 lg:pb-0 border-b lg:border-b-0 border-border">
          {tabs.map((s) => (
            <button
              key={s.id}
              onClick={() => setTab(s.id)}
              className={`flex flex-shrink-0 lg:w-full items-center gap-2 lg:gap-3 rounded-lg px-3.5 py-2.5 text-left text-xs lg:text-sm font-medium whitespace-nowrap transition-colors touch-manipulation active:scale-95 ${
                tab === s.id ? "bg-foreground text-background shadow-sm" : "bg-secondary/40 lg:bg-transparent hover:bg-secondary text-foreground"
              }`}
            >
          ...
          <button
            onClick={signOut}
            className="flex flex-shrink-0 lg:w-full items-center gap-2 lg:gap-3 rounded-lg px-3.5 py-2.5 text-left text-xs lg:text-sm font-medium whitespace-nowrap text-destructive hover:bg-destructive/10 touch-manipulation active:scale-95 lg:mt-2"
          >
